...
def show_sig_only(s):
    s = show_coe_only(s)
    return s[s.applymap(lambda x: x[-1]=='*')].fillna('')
...
def singl_ts(yvar_data, yvar_list, xvar_data, xvar_list, controls=[], intercept=True, HAC=False, maxlags=None):
    ...
    s_cols = ['const']+xvar_list+controls if intercept else xvar_list+controls
    ...
    for yvar in yvar_data[yvar_list]:
        ...
        if intercept:
            X = add_constant(xvar_data[xvar_list+controls]).copy()
        else:
            X = xvar_data[xvar_list+controls].copy()
